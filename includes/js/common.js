...
$(document).ready(function(){
  ...
  function pinsInit () {
    ...
    $.each($(".pin-type-4"), function(i){
      ...
      if ($(".mpp-scaler").length > 0) {
        var duration2 = ($(".trigger-opacity").offset().top-offset.top);
        var tH = gsap.timeline({
          scrollTrigger: {
            trigger: el,
            start: "bottom "+start,
            end: duration2+" "+($("header").outerHeight()),
            duration: 1,
            scrub: true,
            onLeave: function(){
              $("header").removeClass("on-back");
        ...
        var durations = [
          (duration+offset.top-end)/duration2,
          ($(window).outerHeight()-offset.top)/duration2
        ]
        ...
        tH.to("header", {
          duration: 1-(durations[0]+durations[1])
        });
        tH.to("header", {
          opacity: 1,
          display: "block",
    ...
    if ($(".mpp-scaler").length > 0) {
      ...
      var t = gsap.timeline({
        scrollTrigger: {
          ...
          duration: 1,
          onUpdate: function(e){
            if (e.progress >= 0.1 && e.progress <= 0.9 && status != 1) {
              if (!active) {
                if ($(window).width()/$(window).height() <= 1680/1122) {
                  gsap.to(".mpp-object", {scrollLeft: 0, duration: 0.2, ease: Power1.easeInOut});
                }
              ...
              if (active) {
                if ($(window).width()/$(window).height() <= 1680/1122) {
                  gsap.to(".mpp-object", {scrollLeft: (($(window).outerHeight() + $(window).outerHeight()*0.14)*(1.37704918)-($(window).width()+90))*0.2, duration: 0.2, ease: Power1.easeInOut});
                }
  ...
  $("body.body-light .main-bg, body.body-light .main-bg-opacity, .main-bg-overlay").css("height", ($(window).outerHeight() + $(window).outerHeight()*0.14));
  if ($(window).width()/$(window).height() <= 1680/1122) {
    $(".mpp-object .bg").css("height", ($(window).outerHeight() + $(window).outerHeight()*0.14));
    $("body.body-light .bg, body.body-light .main-bg, body.body-light .main-bg-opacity, .mpp-object .bg").css({"background-size": "auto "+($(window).outerHeight() + $(window).outerHeight()*0.14)+"px"});
    ...
    if (mppBg.length > 0) {
      var width = (($(window).outerHeight() + $(window).outerHeight()*0.14)*(1.37704918));
      mppBg.css({
        "width": width+"px"
      });
      if (window.matchMedia("(max-width: 1024px)").matches) {
